...
$PODMAN_CMD run --pull newer --privileged -d --network network -v ${src_dir}/testsuite/dockerfiles/server-all-in-one-dev/mirror:/mirror -v ${src_dir}/testsuite:/testsuite -v /tmp/testing/buildhost_product_uuid:/sys/class/dmi/id/product_uuid -v /tmp/testing:/tmp -v ${src_dir}/testsuite/podman_runner/salt-minion-entry-point.sh:/salt-minion-entry-point.sh --volume /run/dbus/system_bus_socket:/run/dbus/system_bus_socket:ro -v /var/run/docker.sock:/var/run/docker.sock --name buildhost -h buildhost ghcr.io/uyuni-project/uyuni/ci-buildhost:$UYUNI_VERSION bash -c "/salt-minion-entry-point.sh server 1-SUSE-KEY-x86_64"
